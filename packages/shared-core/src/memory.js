...
const createMemoryLogger = (proc, logger) => {
  ...
  const memoryLogger = (label = "") => {
    ...
    logger(
      `${prefix}Memory Usage: ` +
        ...
        (mem.arrayBuffers
          ? ` | Array Buffers: ${formatMB(mem.arrayBuffers)} MB`
          : ""),
    );
  };
  ...
  memoryLogger.getPeakSummary = () => ({
    ...
    arrayBuffers: {
      bytes: peaks.arrayBuffers,
      mb: formatMB(peaks.arrayBuffers),
    },
  });
  ...
  memoryLogger.logPeakSummary = (label = "") => {
    ...
    logger(
      `${prefix}Peak Memory Usage: ` +
        ...
        (peaks.arrayBuffers > 0
          ? ` | Array Buffers: ${formatMB(peaks.arrayBuffers)} MB`
          : ""),
    );
  };
